...
class NonSpikingNeurons(nengo.Process):
    def __init__(self, num_neurons, Cm, bias=None, **kwargs):
...
for num in range(numSamples):
    ...
    with nengo.Network() as model:
        layer = nengo.Node(
            NonSpikingNeuronsWithSynapticInput(int(numNeurons[num]), npModel.c_m, npModel.g_max_non, npModel.del_e,
                                               bias=[current], R=20))
    ...
    pickle.dump(data, open('../../backendSpeed/dataNengoTimesNonspikingSparse.p', 'wb'))
