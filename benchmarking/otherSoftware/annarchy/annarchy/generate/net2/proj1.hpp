...
/////////////////////////////////////////////////////////////////////////////
// proj1: pop1 -> pop1 with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct1 : LILMatrix<int, int> {
    ProjStruct1() : LILMatrix<int, int>( 10, 10) {
    }
        ...
        if(_transmission && _update && pop1._active && ( (t - _update_offset)%_update_period == 0L) ){
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    if(_plasticity){
                    w[i][j] = clip(Gmax[i][j]*(-El[i][j] + pop1.r[rk_pre])/(Eh[i][j] - El[i][j]), 0.0, Gmax[i][j]);
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
