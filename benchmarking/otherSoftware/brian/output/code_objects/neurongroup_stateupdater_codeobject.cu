...
__global__ void
_run_kernel_neurongroup_stateupdater_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_I,
	double* _ptr_array_neurongroup_Ibias,
    ...
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const int _numI = 10;
	const int _numIbias = 10;
	const int _numIsyn = 10;
	const int _numv = 10;
    ...
    const double _lio_1 = 1.0f*dt/0.01;
...

    {
        ...
        const double Ibias = _ptr_array_neurongroup_Ibias[_idx];
        const double Isyn = _ptr_array_neurongroup_Isyn[_idx];
        double v = _ptr_array_neurongroup_v[_idx];
        const double _v = (_lio_1 * (((I + Ibias) + Isyn) - v)) + v;
        v = _v;
...
void _run_neurongroup_stateupdater_codeobject()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_stateupdater_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_stateupdater_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_stateupdater_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_stateupdater_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_stateupdater_codeobject<<<num_blocks, num_threads>>>(
			...
			dev_array_neurongroup_Ibias,
			dev_array_neurongroup_Isyn,
			_array_defaultclock_dt[0],
			dev_array_neurongroup_v
        );

    CUDA_CHECK_ERROR("_run_kernel_neurongroup_stateupdater_codeobject");
