    ...
    def forward(self, x=None):
        ...
        if self.spiking:
            self.theta = self.theta_last + self.time_factor_threshold * (
                        self.theta_leak * (self.theta_0 - self.theta_last) + self.m * (
            ...
            if self.delay:
                ...
                self.g_spike = self.g_spike + torch.minimum((-self.delayed_spikes*self.g_increment), (-self.delayed_spikes)*(self.g_max_spike-self.g_spike))  # Update the conductance of connections which spiked
            else:
                self.g_spike = self.g_spike + torch.minimum((-self.spikes*self.g_increment), (-self.spikes)*(self.g_max_spike-self.g_spike))  # Update the conductance of connections which spiked
            self.V = ((self.V - self.V_reset) * (self.spikes + 1)) + self.V_reset  # Reset the membrane voltages of neurons which spiked
            self.theta = torch.maximum(self.theta_increment, self.theta_floor-self.theta)*(-self.spikes) + self.theta
...
class SNS_Sparse(Backend):
    ...
    def forward(self, x=None):
        ...
        if self.spiking:
            self.theta = self.theta_last + self.time_factor_threshold * (
                        self.theta_leak*(-self.theta_last + self.theta_0) + (self.m.to_dense())[0,
                                                          :] * (self.V_last - self.V_rest))  # Update the firing thresholds
            ...
            self.g_spike = self.g_spike.to_sparse()
            self.V = ((self.V - self.V_reset) * (self.spikes.to_dense() + 1)) + self.V_reset  # Reset the membrane voltages of neurons which spiked
            self.theta = torch.maximum(self.theta_increment, self.theta_floor - self.theta) * (
                -self.spikes.to_dense()) + self.theta
        self.outputs = torch.matmul(self.output_voltage_connectivity, self.V)
        if self.spiking:
        ...
        for nrn in range(self.num_neurons):
            ...
            for syn in range(len(self.incoming_synapses[nrn])):
                ...
                if neuron_src[1]:  # if spiking
                    ...
                    neuron_src[5] = np.maximum(0, np.minimum(neuron_src[3] * ((self.V_last[neuron_src[0]] - neuron_src[6]) / (neuron_src[8] - neuron_src[6])),
                                                             neuron_src[3]))
                    i_syn += neuron_src[5] * (neuron_src[4] - self.V_last[nrn])
            i_gated = 0
            if self.gated:
            ...
            if self.spiking:
                ...
                self.theta[nrn] = self.theta_last[nrn] + self.time_factor_threshold[nrn] * (
                            self.theta_leak[nrn]*(self.theta_last[nrn] + self.theta_0[nrn]) + self.m[nrn] * (self.V_last[nrn] - self.V_rest[nrn]))  # Update the firing thresholds
